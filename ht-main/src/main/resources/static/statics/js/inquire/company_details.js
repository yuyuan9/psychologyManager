...
var box = new Vue({
    ...
    methods: {
        ...
        getContent: function (mun, index) {
            //console.log('查询：' + this.visitedData);
            ...
            //类型
            var istype = this.tabcur;
            if (istype == 0) { istype = '' };
            if (istype == 1) { istype = '认定' };
            if (istype == 2) { istype = '复审' };
            if (istype == 3) { istype = '公示' };
            htajax.postFormDone('/ht-biz/compinfo/list?t=' + new Date().getTime(), {
                ...
                layer.closeAll();
                if (data.code == 10000) {
                    if (data.data.records == '') { box.notfind = 1;  }
                    if (data.data.records != '') { box.notfind = 0; }
