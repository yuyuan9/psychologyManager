...
var box = new Vue({
    ...
    methods: {
        ...
        //访问记录
        visitData: function () {
            htajax.get('/ht-biz/poliprorecord/list?local=' + '中小企业',
                function (data) {
                    //console.log(data);
                    if (data.code == 10000) {
                        box.visitedData = data.data;
                        console.log("中小企业今日查询记录:" + data.data);
                    }
                }, function () {
                    //
                });
        ...
        getContent: function (mun, index) {
            // console.log("剩余honey:" + box.userHoney);
            //jq this对象
            var url3 = this.url3;
            if (url3 == '' || url3 == undefined || url3 == null) {
                var key = $(".keyWord3")[0].id;
                //console.log(key);
                var keyword = key;
            } else {
                var keyword = url3;
            ...
            if (issueData == '选择年份') { issueData = ''; }
            //类型
            var istype = '';
            if (this.tabcur == 0) { istype = '' };
            if (this.tabcur == 1) { istype = '拟入库' };
            if (this.tabcur == 2) { istype = '入库' };
            if (this.tabcur == 3) { istype = '撤销编号' };
            htajax.postFormDone('/sys/ht-biz/rosterinfo/finadll?t=' + new Date().getTime(), {
                enterprise: keyword,//关键字查询
                year: issueData,
                ...
                layer.closeAll();
                if (data.code == 10000) {
                    if (data.data == '') { box.notfind = 1; }
                    if (data.data != '') { box.notfind = 0; }
                    if (data.data == "无结果") { box.notfind = 1; if (keyword == '') { layer.msg("请先输入要查询的关键词") } }
                    ...
                    box.outKeyword = box.getUrlData("keyword3");
                    box.honeyInfo = false;
                } else if (data.code == 1002){//提示free用完，需要消费honey  
                    box.notfind = 1;
                    box.honeyInfo = true;
                } else if (data.code == 1003){
                    box.notfind = 1;
                    box.honeyInfo = true;
                    box.honeyInfoGoon = false;
                }else {
                    box.notfind = 1;
        ...
        //获取用户高企详情查询次数(20后禁止查看详情)
        getLookTime: function (keyword) {
            htajax.get('/ht-biz/compinfo/counts?companyname=' + keyword, function (data) {
                if (data.code == 10000) {
                    console.log(data);
                    box.looktime = data.data;
        ...
        lookmore: function (id, name) {
            ...
            if (box.looktime <= 20) {
                ...
                // 查询企业库
                htajax.get('/ht-biz/compinfo/compnew?companyname=' + keyword, function (data) {
                    if (data.code == 10000) {
                ...
                //查询立项
                htajax.get('/ht-biz/compinfo/projectlib?companyname=' + keyword, function (data) {
                    if (data.code == 10000) {
                        box.contentData2 = data.data;
                    }
                }, function (data) {
                    //layer.closeAll();
                });
            } else {
                layer.msg("20次查询次数已用完");
                return false;
            }
